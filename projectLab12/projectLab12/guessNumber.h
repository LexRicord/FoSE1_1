	...
	while (true) {
		...
		case 1:
			...
			if (enterNumber == number) {
				...
				cout << "Это не то число! Пробуйте дальше!\n";
			break;
		case 2:
			if (countOfHelp == 0) {
			...
			countOfHelp--;
			cout << "Выберите, какого рода подсказка вам нужна:\n1) Это число делиться на\n2) Это число больше числа\n3) Это число меньше числа\n4) Узнать диапазон\n5) Узнать число второго разряда, если он есть\n";
			cin >> help;
			switch (help) {
			case 1:
				int number2;
				cout << "Введите число, на которое, вы думаете, делиться загаданное число: "; cin >> number2;
				if (number % number2 == 0)
					cout << "Это число делиться на " << number2 << endl;
				else
					cout << "Нет, оно не делиться на " << number2 << endl;
				break;
			case 2:
				int number3;
			...
			break;
		case 3:
			cout << "Вы сдались! Задуманной число было " << number << endl;
			active = true;
			break;
		default:
